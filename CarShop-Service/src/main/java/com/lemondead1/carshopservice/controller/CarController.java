...
public class CarController implements Controller {
  ...
  String createCar(User currentUser, CLI cli, String... path) {
    ...
    var car = cars.createCar(currentUser.id(), brand, model, productionYear, price, condition);
    return "Created " + car.prettyFormat() + ".";
  }

  String listCars(User currentUser, CLI cli, String... path) {
    ...
    var availability = cli.parseOptional("Availability for purchase > ", IdListParser.of(Availability.class)).orElse(List.of(Availability.AVAILABLE));
  ...
  String editCar(User currentUser, CLI cli, String... path) {
    if (path.length == 0) {
      throw new WrongUsageException();
    }
    var carToEdit = IntParser.INSTANCE.map(cars::findById).parse(path[0]);

    var newBrand = cli.parseOptional("Brand (" + carToEdit.brand() + ") > ", StringParser.INSTANCE).orElse(null);
    var newModel = cli.parseOptional("Model (" + carToEdit.model() + ") > ", StringParser.INSTANCE).orElse(null);
    var prodYear = cli.parseOptional("Production year (" + carToEdit.productionYear() + ") > ", IntParser.INSTANCE).orElse(null);
    var newPrice = cli.parseOptional("Price (" + carToEdit.price() + ") > ", IntParser.INSTANCE).orElse(null);
    var newCondition = cli.parseOptional("Condition (" + carToEdit.condition() + ") > ", StringParser.INSTANCE).orElse(null);

    var editedCar = cars.editCar(currentUser.id(), carToEdit.id(), newBrand, newModel, prodYear, newPrice, newCondition);
    return "Saved changes to " + editedCar.prettyFormat() + ".";
  }

  String deleteCar(User currentUser, CLI cli, String... path) {
    if (path.length == 0) {
      throw new WrongUsageException();
    }
    var carToDelete = IntParser.INSTANCE.map(cars::findById).parse(path[0]);
    cli.printf("Deleting %s.\n", carToDelete.prettyFormat());
    ...
    try {
      cars.deleteCar(currentUser.id(), carToDelete.id());
      return "Deleted";
    ...
    cars.deleteCarCascading(currentUser.id(), carToDelete.id());
    return "Deleted";
  }
}
