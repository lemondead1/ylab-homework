...
public class CarCreationServletTest extends ServletTest {
  ...
  void doPostCallsCreateCarAndWritesResult() throws IOException {
    ...
    var newCar = new NewCarDTO(car.brand(), car.model(), car.productionYear(), car.price(), car.condition());

    var requestBody = jackson.writeValueAsString(newCar);

    when(carService.createCar(car.brand(), car.model(), car.productionYear(), car.price(), car.condition())).thenReturn(car);
    ...
    verify(carService).createCar(car.brand(), car.model(), car.productionYear(), car.price(), car.condition());
